...
void main() {
  final IWordDatasource datasource = WordDatasourceMock();
  final repository = WordRepository(datasource);
  test('getWord should return a list of Words and has length equals to 2',
      () async {
    when(
      () => datasource.getWord('hi'),
    ).thenAnswer((_) async =>
        (wordsJson as List).map((e) => WordModel.fromMap(e)).toList());

    var result = await repository.getWord('hi');
    final list = result.fold(id, id);

    expect(list, isA<List<Word>>());
